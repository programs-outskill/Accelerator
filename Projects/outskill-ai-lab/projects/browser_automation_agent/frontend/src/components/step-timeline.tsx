import {
  Cpu, Wrench, ArrowRightLeft, CheckCircle2, AlertCircle, CircleDot, FileText,
} from 'lucide-react'
import { cn } from '@/lib/utils'
import type { TimelineEvent } from '@/stores/automation-store'
...
const COLOR_MAP: Record<string, string> = {
  agent_start: 'text-violet-400',
  agent_end: 'text-green-400',
  tool_start: 'text-cyan-400',
  tool_end: 'text-cyan-400/50',
...
function eventLabel(e: TimelineEvent): string {
  switch (e.type) {
    case 'agent_start': return `${e.agent_name} started`
    case 'agent_end': return `${e.agent_name} completed`
    case 'tool_start': return `${e.agent_name} → ${e.detail}`
    case 'tool_end': return `${e.detail} done`
    case 'handoff': return e.detail ?? 'Handoff'
