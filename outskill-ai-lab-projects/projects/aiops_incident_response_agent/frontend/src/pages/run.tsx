...
export function RunPage() {
  ...
  const { scenario, status, phase, report, events, reset } = useIncidentStore()
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-background/80 border-b border-border">
        <div className="flex items-center justify-between px-6 py-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold tracking-tight">AI OPs Incident Response Agent</span>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0 max-w-3xl">
          <div className="mb-6">
            ...
            <p className="text-sm text-muted-foreground">
              {isStreaming
                ? 'Agents are responding to this incident…'
                : status === 'completed'
                  ? 'Incident closed'
                  : 'Starting…'}
            </p>
          ...
          {!report && isStreaming && (
            <div className="flex items-center gap-3 py-12 text-muted-foreground">
              ...
              <span className="text-sm">Incident response agents are working…</span>
            </div>
          )}
