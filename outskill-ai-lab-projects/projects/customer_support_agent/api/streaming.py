"""SSE streaming infrastructure for the Customer Support Agent API.
...
from customer_support_agent.api.schemas.support import PhaseType, RunStatus
from customer_support_agent.simulators.scenario_engine import ScenarioType

logger = logging.getLogger(__name__)

AGENT_PHASE_MAP: dict[str, PhaseType] = {
    "Intake & Router Agent": "intake",
    "Order Support Agent": "order_support",
    "Billing Support Agent": "billing_support",
    "Technical Support Agent": "technical_support",
    "Escalation Agent": "escalation",
    "Resolution & CSAT Agent": "resolution",
}


@dataclass
class RunState:
    """In-memory state for a single customer support run.
    ...
    Attributes:
        ...
        report: Final resolution text once completed.
...
def create_run(scenario_type: ScenarioType) -> RunState:
    ...
    Args:
        scenario_type: The support scenario to simulate.
...
class StreamingSupportHooks(AgentHooks):
